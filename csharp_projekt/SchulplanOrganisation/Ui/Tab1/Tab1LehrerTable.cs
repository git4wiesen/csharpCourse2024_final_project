...
{
    public partial class Tab1LehrerTable : UserControl
    {
        private readonly string dbFile;
        private readonly string personTable = "Person";
        private readonly string schuelerTable = "Lehrer";
        private readonly string klasseSortString = "'Nachname' ASC, 'Erster Vorname' ASC, 'Andere Vornamen' ASC, 'Geburtstag' ASC";
        private readonly string[] klasseColumns = ["Lehrer.ID", "Person.ID", "Person.nachname", "Person.ersterVorname", "Person.andereVornamen", "Person.geburtstag"];
        private readonly string[] klasseColumnHeaderText = ["LID", "PID", "Nachname", "Erster Vorname", "Andere Vornamen", "Geburtstag"];
        ...
        public Tab1LehrerTable(string dbFile)
        {
            InitializeComponent();
            this.dbFile = dbFile;

            lblTitle.Text = "Lehrer";
        ...
        public void DeleteRow()
        {
            ...
            if (MessageBox.Show(
                mainForm,
                $"Möchtest du den Lehrer '{drv["'Nachname'"]}, {drv["'Erster Vorname'"]}{(!string.IsNullOrWhiteSpace(andereVornamen) ? $" {andereVornamen}" : "")}' wirklich löschen?",
                "Lehrer löschen?",
                                ...
                                case DataRowState.Added:
                                    {
                                        ...
                                        commandUpdateSchueler.CommandText = "INSERT INTO Lehrer (Person) VALUES (@PID)";
                                ...
                                case DataRowState.Deleted:
                                    {
                                        ...
                                        commandDeleteSchueler.CommandText = "DELETE FROM Lehrer WHERE ID = @LID";
                                        commandDeleteSchueler.Parameters.Add("@LID", OleDbType.Integer).Value = row["'LID'", DataRowVersion.Original];
        ...
        private void LoadDatabase()
        {
            ...
            try
            {
                ...
                command.CommandText = string.Format(
                    "SELECT {0} FROM {1}, {2} WHERE Person.ID = Lehrer.Person",
                    string.Join(", ", columnInfo.Select(info => $"{info.ColumnName} AS '{info.ColumnTitleText}'")),
                    tableName1, tableName2
                );
                using OleDbDataAdapter dataAdapter = this.dataAdapter = new OleDbDataAdapter(command);
                ...
                try
                {
                    ...
                    grid.DataMember = "lehrer";
                    grid.DataSource = dv;

                    grid.Columns["'LID'"].Visible = false;
